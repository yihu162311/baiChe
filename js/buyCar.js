			...
			option.options.forEach(function(item){
				...
				li.onclick = function(){
					renderCars(carData.filter(function(_item){
						return option.filterFunc(_item, item.value);   /*返回一个方法,此方法用来实现对信息的筛选       _item是carData,item.value是filterSetting.options.value*/
					}));
...
filterSetting.forEach(function(item, index){
	item.filterFunc = filterFunc[index]; // 给对象一个新的属性:filterFunc
